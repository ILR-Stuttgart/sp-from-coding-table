...
NTRIM <- 23 # how many verbs the inexperienced learner knows
```
...
ep <- nmplot(learner="ep", time=max(tpt.plot$Time), df=tpt.plot) + ylim(NA, 90)
ip <- nmplot(learner="ip", time=max(tpt.plot$Time), df=tpt.plot) + ylim(NA, 90)
plot_grid(
  ep, ip,
  ncol = 2, # two columns
  nrow = 1,
  labels = c("Full lexicon", paste("Top ", as.character(NTRIM), " types")) # Plot titles
)
...
es <- nmplot.time(learner="es", tpt.trimmed)
is <- nmplot.time(learner="is", tpt.trimmed)
